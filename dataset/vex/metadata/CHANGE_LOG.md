...
- merges externally generated silver-label predictions back into the master dataset via `grading_id`,
...
Note: the current repository references `v1_0_stable.parquet` throughout the modeling code, but the dedicated `v1_0.py` release script is empty in this snapshot. So `v1_0` can be documented confidently as the released frozen dataset, while the exact final conversion step is not fully scripted in the public files here.
